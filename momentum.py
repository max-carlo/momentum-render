...
def scrape_finviz_news(tic):
    ...
    return [
        ...
        for r in rows
        if (td := r.find("td", width="130"))
        and (a := r.find("a", class_="tab-link-news"))
        and (sp := r.find("span"))
    ]
...
# ---------------- SEC-Edgar EPS (alle Einträge) ----------------
@st.cache_data(ttl=86400)
def get_sec_eps_raw(tic):
    # Ticker → CIK
    mapping = requests.get("https://www.sec.gov/files/company_tickers.json",
                           headers={"User-Agent": "Mozilla/5.0"}).json()
    cik = next(
        (str(v["cik_str"]).zfill(10) for v in mapping.values()
         if v["ticker"].upper() == tic.upper()),
        None
    )
    if not cik:
