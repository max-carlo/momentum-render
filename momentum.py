...
# ============================================================
# 6) SEC Edgar EPS (roh, keine Filter)
# ============================================================

@st.cache_data(ttl=86400)
def get_sec_eps_raw(tic:str):
    mapping=requests.get("https://www.sec.gov/files/company_tickers.json",headers={"User-Agent":"Mozilla/5.0"}).json()
    ...
    facts=requests.get(f"https://data.sec.gov/api/xbrl/companyfacts/CIK{cik}.json",headers={"User-Agent":"Mozilla/5.0"}).json()
    units=facts["facts"]["us-gaap"]["EarningsPerShareBasic"]["units"]
    unit_vals=next(iter(units.values()))
    rows=[]
    for e in unit_vals:
        if e.get("fp","").startswith("Q"):
            try: rows.append((datetime.datetime.fromisoformat(e["end"]),e["val"]))
            except: pass
    df=pd.DataFrame(rows,columns=["Period","EPS Actual"])
    df.sort_values("Period",ascending=False,inplace=True)
    df["year"]=df["Period"].dt.year; df["quarter"]=df["Period"].dt.quarter
